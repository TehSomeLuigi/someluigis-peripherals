...
/**
 * The <code>Listener</code> object is represents the interface to the server
 * that the clients can connect to. This is responsible for making call backs to
 * the <code>Acceptor</code> when there is a new connection waiting to be
 * accepted. When the connection is to be closed the interface object can be
 * closed.
 * 
...
class SocketListener implements Closeable {
     ...
     */
    public SocketListener(SocketAddress address, Server server, Agent agent)
            throws IOException {
    ...
    /**
     * This is used to register the <code>Acceptor</code> to listen for new
     * connections that are ready to be accepted. Once this is registered it
     * will remain registered until the interface is closed, at which point the
     * socket is closed.
     * 
     * @throws IOException
     *             thrown if there is a problem registering
     */
    private void process() throws IOException {
        try {
            this.reactor.process(this.acceptor, OP_ACCEPT);
     ...
     * connections. This will perform a close of the connected server socket
     * that have been created from using the <code>Acceptor</code> object.
     * 
     * @throws IOException
