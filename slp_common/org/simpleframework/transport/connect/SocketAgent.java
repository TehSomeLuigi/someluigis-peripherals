...
/**
 * The <code>SocketAgent</code> is used to wrap an agent for safety. Wrapping an
 * agent in this way ensures that even if the agent is badly written there is
 * little chance that it will affect the operation of the server. All
 * <code>Trace</code> objects returned from this will catch all exceptions
 * within the created trace.
 * 
...
class SocketAgent implements Agent {
    ...
    /**
     * Constructor for the <code>SocketAgent</code> object. This will be given
     * the agent that is to be used to create traces. This can be a null value,
     * in which case the trace provided will be a simple empty void that
     * swallows all trace events.
     * 
     * @param agent
     *            the agent that is to be wrapped by this
     */
    ...
    public Trace attach(SocketChannel channel) {
        ...
        if (this.agent != null) {
            trace = this.agent.attach(channel);
        }
