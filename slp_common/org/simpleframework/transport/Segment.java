...
/**
 * The <code>Segment</code> object is used to provide a means to remove a packet
 * from an segment builder when closed. This can be used to wrap an existing
 * packet and add functionality to it. Once closed this segment should no longer
 * be used.
 * 
...
class Segment implements Packet {
    ...
    /**
     * Constructor for the <code>Segment</code> object. This is used to create a
     * packet that wraps another packet, and removes that packet from the head
     * of a given queue when it is closed.
     * 
     * @param packet
     *            this is the packet that is to be wrapped
     * @param queue
     *            this is the queue to remove the packet from
     */
     ...
     * it tells the writer whether it needs to block the writing thread while
