...
/**
 * The <code>SocketTransport</code> object offers a transport that can send and
 * receive bytes in a non-blocking manner. The contract of the
 * <code>Transport</code> is that it must either write the data it is asked to
 * write or it must queue that data for delivery. For the vast majority of cases
 * data is written directly to the socket without any need for queuing or
 * selection for write ready events.
 * <p>
 * In the event that the client TCP window is full and writing would block this
 * makes use of a queue of buffers which can be used to append data to. The
 * buffers are lazily instantiated so the memory required is created only in the
 * rare case that they are needed. Once a buffer is full it is queued to an
 * asynchronous thread where the buffer queue is drained and sent to the client
 * when the TCP window of the client is capable of accepting it.
 * <p>
 * In order to improve the network performance of this transport the default
 * packet size sent to the TCP stack is four kilobytes. This ensures that the
 * fragments of response delivered to the TCP layer are sufficiently large for
...
public class SocketTransport implements Transport {
    ...
    /**
     * Constructor for the <code>SocketTransport</code> object. This requires a
     * reactor to perform asynchronous writes and also the pipeline which is
     * used to read and write data. This transport will use a queue of buffers
     * which are lazily initialized so as to only allocate the memory on demand.
     * 
     * @param socket
     *            this is used to read and write the data
     * @param reactor
     ...
     */
    public SocketTransport(Socket socket, Reactor reactor, int limit)
            throws IOException {
     ...
     *            this is the threshold for asynchronous buffers
     * @param queue
     *            this is the queue size for asynchronous writes
     */
    public SocketTransport(Socket socket, Reactor reactor, int limit, int queue)
            throws IOException {
     ...
     * encoded for SSL transport or send directly. This will buffer the bytes
     * within the internal buffer to ensure that the response fragments are
     * sufficiently large for the network. Smaller packets result poorer
     * performance.
     * 
     * @param data
     ...
     * Flushing with this method is always non-blocking, so if the socket is not
     * write ready and the buffer can be queued it will be queued and the
     * calling thread will return.
     */
    @Override
    ...
    /**
     * This method is used to flush the internal buffer and close the underlying
     * socket. This method will not complete until all buffered data is written
     * and the underlying socket is closed at which point this can be disposed
     * of.
     */
    @Override
