...
/**
 * The <code>Appender</code> represents a packet that wraps a pooled byte
 * buffer. This implementation provides write access to the underlying buffer,
 * so it can modify its contents if required. This means that the
 * <code>append</code> methods can add data to the buffer, and that the buffer
 * can be compacted if required.
 * 
...
class Appender implements Packet {
    ...
    /**
     * Constructor for the <code>Appender</code> object. This creates a packet
     * with a recyclable buffer. This appender can not be recycled as it is
     * provided with no recycler.
     * 
     * @param buffer
     *            this is the buffer used by this packet
     * @param sequence
     *            this is the unique sequence number for this
     */
    ...
    /**
     * Constructor for the <code>Appender</code> object. This creates a packet
     * with a recyclable buffer. The buffer provided can be modified up until
     * such point as it is recycled. To recycle the buffer the packet must be
     * closed.
     * 
     * @param buffer
     *            this is the buffer used by this packet
     * @param manager
     *            this is the buffer pool to pass the buffer to
     * @param sequence
     *            this is the unique sequence number for this
     */
     ...
     * packet. This is typically equivalent to capacity minus the length.
    ...
    /**
     * This is used to determine how mnay bytes remain within this packet. It
     ...
     *            this is the character set to use for encoding
     * @param segment
     *            this is the buffer that is to be encoded
     * 
     * @return this returns the bytes sequence as a string object
     */
    private String encode(String encoding, ByteBuffer segment)
            throws IOException {
    ...
    public int append(ByteBuffer data, int count) throws IOException {
        ...
        if (this.closed) throw new PacketException("Packet has been closed");
     ...
     *            this is the channel to write the packet to
     * @param segment
     *            this is the buffer that is to be written
     * 
     * @return this returns the number of bytes that were written
     */
     ...
     * resources occupied by the packet are released. The resources held by this
     * instance include pooled buffers. If the packet is not closed on
    ...
    public void close() {
        if (this.manager != null) {
            this.manager.recycle(this.buffer);
        }
        this.manager = null;
        this.closed = true;
    }
