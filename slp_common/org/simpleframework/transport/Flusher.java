...
/**
 * The <code>Flusher</code> object is used to flush data to the underlying
 * socket. This allows asynchronous writes to the socket to be managed in such a
 * way that there is order to the way data is delivered over the socket. This
 * uses a selector to dispatch flush invocations to the underlying socket when
 * the socket is read ready. This allows the writing thread to continue without
...
interface Flusher {
     ...
     * fully flush the content to the underlying transport. This will block if
