...
/**
 * The <code>TransportSource</code> object represents a cursor that can read and
...
class TransportSource implements Source {
    ...
    /**
     * Constructor for the <code>TransportSource</code> object. This requires a
     * transport to read the bytes from. By default this will create a buffer of
     * two kilobytes to read the input in to which ensures several requests can
     * be read at once.
     * 
     * @param transport
    ...
    /**
     * Determines whether the source is still open. The source is considered
     * open if there are still bytes to read. If there is still bytes buffered
     * and the underlying transport is closed then the source is still
     * considered open.
     * 
    ...
    /**
     * Determines whether the source is ready for reading. When the source is
    ...
    public int read(byte[] data, int off, int len) throws IOException {
        ...
        int size = Math.min(len, this.count); // get the minimum

        if (size > 0) {
            this.buffer.get(data, off, size); // get the bytes
            this.count -= size;
        }
     ...
     * within the stream. When this reaches zero then the source may perform a
     * blocking read.
     * 
    ...
    private int peek() throws IOException {
        if (this.count <= 0) { // reset the buffer for filling
            this.buffer.clear();
        }
        if (this.count > 0) {
            this.buffer.compact(); // compact the buffer
        }
        this.count += this.transport.read(this.buffer); // how many were read

        if (this.count > 0) {
            this.buffer.flip(); // if there is something then flip
        }
        if (this.count < 0) { // close when stream is fully read
            this.close();
        }
        return this.count;
    }
    ...
    /**
     * Moves the source backward within the stream. This ensures that any bytes
    ...
    /**
     * This is used to close the underlying transport. This is used when the
     * transport returns a negative value, indicating that the client has closed
     * the connection on the other side. If this is invoked the read method
     * returns -1 and the cursor is not longer open, further bytes can no longer
     * be read.
     */
