...
/**
 * The <code>RequestConsumer</code> object is used to parse the HTTP request
 * line followed by the HTTP message headers. This parses the request URI such
 * that the query parameters and path are extracted and normalized. It performs
 * this using external parsers, which will remove and escaped characters and
 * normalize the path segments. Finally this exposes the HTTP version used using
 * the major and minor numbers sent with the HTTP request.
 * 
...
public class RequestConsumer extends HeaderConsumer {
    ...
    /**
     * Constructor for the <code>RequestConsumer</code> object. This is used to
     * create a consumer which can consume a HTTP request header and provide the
     * consumed contents via a known interface. This also further breaks down
     * the request URI for convenience.
     */
    ...
    /**
     * This can be used to get the target specified for this HTTP request. This
     * corresponds to the URI sent in the request line. Typically this will be
     * the path part of the URI, but can be the full URI if the request is a
     * proxy request.
     * 
    ...
    /**
     * This method returns a <code>CharSequence</code> holding the data consumed
     * for the request. A character sequence is returned as it can provide a
     * much more efficient means of representing the header data by just
     * wrapping the consumed byte array.
     * 
     ...
     * allows various parameters to be acquired without having to parse the
     * target.
     * 
     ...
     * Method Definitions. Typically this will be a GET or POST method, but can
     * be any valid alphabetic token.
     * 
     ...
     * version corrosponds to the major protocol type, that is the 1 of a
     * HTTP/1.0 version string. Typically the major type is 1, by can be 0 for
     * HTTP/0.9 clients.
     * 
     ...
     * version corrosponds to the minor protocol type, that is the 0 of a
     * HTTP/1.0 version string. This number is typically used to determine
     * whether persistent connections are supported.
     * 
     ...
     * such that it can be used by the subclass for some useful puropse. This is
    ...
    /**
     * This will parse URI target from the first line of the header and store
     * the parsed string internally. The target token is used to create an
     * <code>Address</code> object which provides all the details of the target
    ...
    /**
     * This will parse HTTP method from the first line of the header and store
     * the parsed string internally. The method is used to determine what action
     * to take with the request, it also acts as a means to determine the
    ...
    /**
     * This will parse HTTP version from the first line of the header and store
     * the parsed string internally. The method is used to determine what
     * version of HTTP is being used. Typically this will be HTTP/1.1 however
     * HTTP/1.0 must be supported and this has different connection semantics
     * with regards to pipelines.
     */
    ...
    /**
     * This will parse the header from the current offset and convert the bytes
     * found into an int as it parses the digits it comes accross. This will
     * cease to parse bytes when it encounters a non digit byte or the end of
     * the readable bytes.
     */
    private void major() {
        while (this.pos < this.count) {
            if (!this.digit(this.array[this.pos])) {
                break;
            }
            this.major *= 10;
     ...
     * character, between an ISO-8859-1 0 and 9. If it is, this will return true
     * otherwise it returns false.
     * 
     * @param octet
     *            this is to be checked to see if it is a digit
     * 
    ...
    /**
     * This method returns a string representing the header that was consumed by
     * this consumer. For performance reasons it is better to acquire the
    ...
    private class Token {
        ...
        public String toString(String charset) {
            try {
                return new String(RequestConsumer.this.array, this.off,
                        this.size, charset);
            } catch (IOException e) {
                return null;
            }
     ...
     * Here the internal byte buffer is simply wrapped so that it can be a
     * represented as a <code>CharSequence</code>. Wrapping the consumed array
     * in this manner ensures that no further memory allocation is required.
     */
    private class ByteSequence implements CharSequence {

        /**
         * This returns the length of the header in bytes. The length includes
         * the request line and all of the control characters including the
         * carriage return and line feed at the end of the request header.
         * 
         * @return this returns the number of bytes for the header
         */
        @Override
        public int length() {
            return RequestConsumer.this.count;
        }

        /**
         * This is used to acquire the character at the specified index.
         * Characters returned from this method are simply the bytes casted to a
         * character. This may not convert the character correctly and a more
         * sensible method should be used.
         * 
         * @param index
         *            the index to extract the character from
         * 
         * @return this returns the character found at the index
         */
        @Override
        public char charAt(int index) {
            return (char) RequestConsumer.this.array[index];
        }

        /**
         * This returns a section of characters within the specified range.
         * Acquiring a section in this manner is simply done by setting a start
         * and end offset within the internal array.
         * 
         * @param start
         *            this is the start index to be used
         * @param end
         *            this is the end index to be used
         * 
        ...
        /**
         * This is used to create a string from the header bytes. This converts
         * the header bytes to a string using a compatible encoding. This may
         * produce different results depending on the time it is invoked, as the
         * header consumes more data.
         * 
         ...
         * 
         * @param charset
         *            this is the encoding to use for the header
         * 
         * @return this returns an encoded version of the header
         */
        public String toString(String charset) {
            try {
                return new String(RequestConsumer.this.array, 0,
                        RequestConsumer.this.count, charset);
            } catch (Exception e) {
                return null;
            }
        }
         ...
         * consumed. Here the internal byte buffer is simply wrapped so that it
         * can be a represented as a <code>CharSequence</code>. Wrapping the
         * consumed array in this manner ensures that no further memory
        ...
        private class Section implements CharSequence {
            ...
            /**
             * Constructor for the <code>Section</code> object. This is given
             * the start offset and the number of bytes the section is to
             * contain, this directly references the internal buffer.
             * 
             * @param off
             *            the start offset within the header
             * @param count
             *            the number of bytes within the header
             */
             ...
             * including the carriage return and line feed at the end of the
             * request header.
             * 
             * @return this returns the number of bytes for the header
             */
            ...
            /**
             * This is used to acquire the character at the specified index.
             * Characters returned from this method are simply the bytes casted
             * to a character. This may not convert the character correctly and
             * a more sensible method should be used.
             * 
             * @param index
             *            the index to extract the character from
             * 
            ...
            @Override
            public char charAt(int index) {
                return (char) RequestConsumer.this.array[this.off + index];
            }

            /**
             * This returns a section of characters within the specified range.
             * Acquiring a section in this manner is simply done by setting a
             * start and end offset within the internal array.
             * 
             * @param start
             *            this is the start index to be used
             * @param end
            ...
            @Override
            public CharSequence subSequence(int start, int end) {
                return new Section(this.off + start, start - end);
            }

            /**
             * This is used to create a string from the header bytes. This
             * converts the header bytes to a string using a compatible
             * encoding. This may produce different results depending on the
             * time it is invoked, as the header consumes more data.
             * 
             * @return this returns an encoded version of the header
             */
            @Override
