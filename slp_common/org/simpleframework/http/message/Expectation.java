...
/**
 * The <code>Expectation</code> object is used to send the expect continue
 * status if required. This is delivered to the client to tell the client that
 * the server is willing to accept the request body. Once this is sent the
 * transport will likely wait until there is a read ready event.
 * 
...
class Expectation {
     ...
     */
    private static final byte[] STATUS = { 'H', 'T', 'T', 'P', '/', '1', '.',
            '1', ' ', '1', '0', '0', ' ' };

    /**
     * This is the optional description for the expect status code.
     */
    private static final byte[] MESSAGE = { 'C', 'o', 'n', 't', 'i', 'n', 'u',
            'e', '\r', '\n', '\r', '\n' };
    ...
    /**
     * This will execute the continue if the header contains the expectation
     * header. If there is no expectation then this will return without sending
     * anything back to the connected client.
     * 
     * @param header
     *            this is the header read from the channel
     */
    public void execute(Header header) throws IOException {
        if (header.isExpectContinue()) {
            this.trace.trace(EXPECT_CONTINUE);
