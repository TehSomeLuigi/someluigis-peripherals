...
/**
 * The <code>Transfer</code> object acts as a means to determine the transfer
 * encoding for the response body. This will ensure that the correct HTTP
 * headers are used when the transfer of the body begins. In order to determine
 * what headers to use this can be provided with a content length value. If the
 * <code>start</code> method is provided with the content length then the HTTP
 * headers will use a Content-Length header as the message delimiter. If there
 * is no content length provided then the chunked encoding is used for HTTP/1.1
 * and connection close is used for HTTP/1.0.
 * 
...
class Transfer {
    ...
    /**
     * This is used to determine if the transfer has started. It has started
     * when a producer is created and the HTTP headers have been sent, or at
     * least handed to the underlying transport. Once started the semantics of
     * the connection can not change.
     * 
    ...
    /**
     * This starts the transfer with no specific content length set. This is
     * typically used when dynamic data is emitted ans will require chunked
     * encoding for HTTP/1.1 and connection close for HTTP/1.0. Once invoked the
     * HTTP headers are committed.
     */
    ...
    /**
     * This starts the transfer with a known content length. This is used when
     * there is a Content-Length header set. This will not encode the content
     * for HTTP/1.1 however, HTTP/1.0 may need a connection close if it does not
     * have keep alive semantics.
     * 
     * @param length
     *            this is the length of the response body
     */
    ...
    /**
     * This method is used to write content to the underlying socket. This will
     * make use of the <code>Producer</code> object to encode the response body
     * as required. If the producer has not been created then this will throw an
     * exception.
     * 
     * @param array
    ...
    public void write(byte[] array, int off, int len) throws IOException {
        ...
        this.trace.trace(WRITE_BODY, len);
     ...
     * This will check the existing HTTP headers, and if there is insufficient
     * data chunked encoding will be used for HTTP/1.1 and connection close will
     * be used for HTTP/1.0.
     */
    private void configure() throws IOException {
        ...
        if (empty) {
            this.support.setContentLength(0);
        } else if (length >= 0) {
     ...
     * 
     * @param count
     *            this is the number of bytes to be transferred
     */
     ...
     *            this is the number of bytes to be transferred
     * @param length
     *            this is the actual length value to be used
     */
    ...
    /**
     * This is used to clear any previous encoding that has been set in the
     * event that content length may be used instead. This is used so that an
     * override can be made to the transfer encoding such that content length
     * can be used instead.
     */
    ...
    /**
     * This is used to compose the HTTP header and send it over the transport to
     * the client. Once done the response is committed and no more headers can
     * be set, also the semantics of the response have been committed and the
     * producer is created.
     */
