...
/**
 * The <code>FixedProducer</code> object produces content without any encoding,
 * but limited to a fixed number of bytes. This is used if the length of the
 * content being delivered is know beforehand. It will simply count the number
 * of bytes being send and signal the server kernel that the next request is
 * ready to read once all of the bytes have been sent to the client.
 * 
...
class FixedProducer implements Producer {
    ...
    /**
     * Constructor for the <code>FixedProducer</code> object. This is used to
     * create a producer that will count the number of bytes that are sent over
     * the pipeline, once all bytes have been sent this will signal that the
     * next request is ready to read.
     * 
     * @param sender
     ...
     * @param limit
     *            this is used to limit the number of bytes sent
     */
    ...
    public void produce(ByteBuffer buffer, int off, int len) throws IOException {
        long size = Math.min(len, this.limit - this.count);

        try {
            if (this.monitor.isClosed())
                throw new ProducerException("Response content complete");
            this.sender.send(buffer, off, (int) size);
