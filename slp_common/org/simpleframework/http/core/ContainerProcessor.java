...
/**
 * The <code>ContainerProcessor</code> object is used to create channels which
 * can be used to consume and process requests. This is basically an adapter to
 * the <code>Selector</code> which will convert the provided transport to a
 * usable channel. Each of the connected pipelines will end up at this object,
 * regardless of whether those connections are SSL or plain data.
 * 
...
public class ContainerProcessor implements Processor {
    ...
    /**
     * Constructor for the <code>ContainerProcessor</code> object. This is used
     * to create a processor which will convert the provided transport objects
     * to channels, which can then be processed by the selector and dispatched
     * to the container.
     * 
     * @param container
     *            the container to dispatch requests to
     * @param allocator
     *            this is the allocator used to buffer data
     * @param count
     *            this is the number of threads to be used
     */
    public ContainerProcessor(Container container, Allocator allocator,
     ...
     */
    public ContainerProcessor(Container container, Allocator allocator,
            int count, int select) throws IOException {
    ...
    /**
     * This is used to process the requests from a provided transport and
     * deliver a response to those requests. A transport can be a direct
     * transport or a secure transport providing SSL.
     * <p>
     * Typical usage of this method is to accept multiple transport objects,
     * each representing a unique HTTP channel to the client, and process
     * requests from those transports concurrently.
     * 
     * @param transport
