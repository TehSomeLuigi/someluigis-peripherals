...
/**
 * The <code>Monitor</code> object is core to how the requests are processed
 * from a pipeline. This monitors the progress of the response streams as they
 * are written to the underlying transport which is typically TCP. If at any
 * point there is an error in the delivery of the response the monitor is
 * notified. It can then shutdown the connection, as RFC 2616 suggests on
 * errors.
 * <p>
 * If however the response is delivered successfully the monitor is notified of
 * this event. On successful delivery the monitor will hand the
 * <code>Channel</code> back to the server kernel so that the next request can
 * be processed. This ensures ordering of the responses matches ordering of the
 * requests.
 * 
