...
/**
 * This object is used to represent a HTTP request. This defines the attributes
 * that a HTTP request has such as a request line and the headers that come with
 * the message header.
 * <p>
 * The <code>Request</code> is used to provide an interface to the HTTP
 * <code>InputStream</code> and message header. The stream can have certain
 * characteristics, these characteristics are available by this object. The
 * <code>Request</code> provides methods that allow the <code>InputStream</code>
 * 's semantics to be known, for example if the stream is keep-alive or if the
 * stream has a length.
 * <p>
 * The <code>Request</code> origin is also retrievable from the
 * <code>Request</code> as is the attributes <code>Map</code> object which
 * defines specific connection attributes. And acts as a simple model for the
 * request transaction.
 * <p>
 * It is important to note that the <code>Request</code> controls the processing
 * of the HTTP pipeline. The next HTTP request is not processed until the
 * request has read all of the content body within the <code>InputStream</code>.
...
class RequestEntity extends RequestMessage implements Request {
    ...
    /**
     * Constructor for the <code>RequestEntity</code> object. This is used to
     * create a request that contains all the parts sent by the client,
     * including the headers and the request body. Each of the request elements
     * are accessible through this object in a convenient manner, all parts and
     * parameters, as well as cookies can be accessed and used without much
     * effort.
     * 
     * @param entity
     *            this is the entity that was sent by the client
     * @param monitor
     *            this is the monitor used to monitor events
     */
     ...
     * this message has the Connection header with the close token. If the close
     * token is present then this stream is not a keep-alive connection. However
     * if this has no Connection header then the keep alive status is determined
     * by the HTTP version, that is HTTP/1.1 is keep alive by default, HTTP/1.0
     * has the connection close by default.
     * 
    ...
    public boolean isKeepAlive() {
        ...
        else if (this.getMajor() > 1)
            return true;
        else if (this.getMajor() == 1) return this.getMinor() > 0;
        return false;
    }
    ...
    public SSLSession getSecuritySession() {
        ...
        if (engine != null) return engine.getSession();
        return null;
    }
     ...
     * 
     * @param socket
     *            this is the socket to get the address for
     * 
     * @return this returns the client address for this request
     */
    ...
    public String getContent() throws IOException {
        ...
        if (type == null) return this.body.getContent("ISO-8859-1");
     ...
     * 
     * @param type
     *            this is the content type used with the request
     * 
    ...
    public ReadableByteChannel getByteChannel() throws IOException {
        ...
        if (source != null) return Channels.newChannel(source);
        return null;
    }
