...
/**
 * The <code>ProducerFactory</code> is used to create a producer to match the
 * HTTP header sent with the response. This interprets the headers within the
 * response and composes a producer that will match those. Producers can be
 * created to send in chunked encoding format, as well as fixed length and
...
class ProducerFactory {
    ...
    /**
     * This is used to create an a <code>Producer</code> object that can be used
     * to produce content according to the HTTP header. If the request was from
     * a HTTP/1.0 client that did not ask for keep alive connection semantics a
     * simple close producer is created. Otherwise the content is chunked
     * encoded or sent according the the Content-Length.
     * 
    ...
    public Producer getInstance() {
        ...
        if (!keepAlive) return new CloseProducer(this.sender, this.monitor);
     ...
     * 
     * @param chunkable
     *            does the connected client support chunked
     * 
     * @return this returns the producer used to send the response
     */
