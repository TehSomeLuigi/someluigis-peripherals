...
/**
 * The <code>CloseProducer</code> is used to close a connection once all of the
 * content has been produced. This is typically used if the connected client
 * supports the HTTP/1.0 protocol and there is no Connection header with the
 * keep-alive token. For reasons of performance this should not be used for
 * HTTP/1.1 clients.
 * 
...
class CloseProducer implements Producer {
    ...
    /**
     * Constructor for the <code>CloseProducer</code> object. This is used to
     * create a producer that will close the underlying socket as a means to
     * signal that the response is fully sent. This is typically used with
     ...
     * and the user no longer needs to write. This will close the underlying
     * transport which tells the client that all of the content has been sent
     * over the connection.
     */
    @Override
