...
/**
 * The <code>FilterAllocator</code> object is used to provide a means to provide
 * a general set of constraints around buffer allocation. It can ensure that a
 * minimum capacity is used for default allocation and that an upper limit is
 * used for allocation. In general this can be used in conjunction with another
 * <code>Allocator</code> which may not have such constraints. It ensures that a
 * set of requirements can be observed when allocating buffers.
 * 
...
public class FilterAllocator implements Allocator {
    ...
    /**
     * Constructor for the <code>FilterAllocator</code> object. This is used to
