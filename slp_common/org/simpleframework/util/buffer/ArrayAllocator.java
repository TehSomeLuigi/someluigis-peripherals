...
/**
 * The <code>ArrayAllocator</code> object is used to provide a means to allocate
 * buffers using a single byte array. This essentially uses the heap to allocate
 * all buffers. As a result the performance of the resulting buffers is good,
 * however for very large buffers this will use quote allot of the usable heap
 * space. For very large buffers a mapped region of shared memory of a file
...
public class ArrayAllocator implements Allocator {
    ...
    /**
     * Constructor for the <code>ArrayAllocator</code> object. This is used to
    ...
    public Buffer allocate(long size) throws IOException {
        ...
        return new ArrayBuffer(required, this.limit);
    }
