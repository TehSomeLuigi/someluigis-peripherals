...
/**
 * The <code>Lease</code> object is used to keep a keyed resource active. This
 * provides a very simple lease that can be used to track the activity of a
 * resource or system. Keeping track of activity allows resources to be
 * maintained until such time that they are no longer required, allowing the
 * server to clean up any allocated memory, file descriptors, or other such
 * data.
 * 
...
public interface Lease<T> {
     ...
     * 
     * @exception Exception
     *                if the expiry could not be acquired
     */
     ...
     * degree of accuracy. The accuracy of the leasing system is dependent on
